...
public class MemberService {
	...
	// 현재 프로젝트의 특정 폴더를 위치 기준으로 지정하므로, 절대경로로 작성하지 않아도 된다
	// JSP에서는 application 내장객체를 통해 위치를 참조하는 방식이 있다
	...
	public String getAgreement() {
		...
		String data = "";
		try {
			 File f = agreement.getFile(); // new File() 대신 스프링이 찾아낸 자원을 파일의 형태로 반환
			 sc = new Scanner(f);
	...
	public String renewPassword(MemberDTO dto) throws Exception {
...
//			int i = ran.nextInt(chars.length()); 	// 글자수의 범위안에서 랜덤 정수를 받아온다
...
//		}
		...
		String newPassword = UUID.randomUUID().toString().substring(0, 8).toLowerCase();
	...
	public int modifyMember(MemberDTO dto) throws Exception {
		// "".equals(dto.getUserpw() : 빈문자열과 dto의 userpw값이 일치한다 ( == false)
		if(dto.getUserpw().equals("") == false) {			// 비밀번호가 빈값이 아닐 경우에만
			dto.setUserpw(hash.getHash(dto.getUserpw()));	// hash 처리해서 dao로 넘김
		}
		return dao.updateMember(dto);
	}

	public int withdraw(String userid, String userpw) throws Exception {
		// 마이바티스에 파라미터를 넘길때는 하나의 객체로 묶어서 전달하는것이 좋다
		HashMap<String, Object> param = new HashMap<String, Object>(2);
		param.put("userid", userid);	// 아이디는 DB에 평문으로 저장되어 있다
		param.put("userpw", hash.getHash(userpw));	// 비밀번호는 해쉬로 저장되어 있다
