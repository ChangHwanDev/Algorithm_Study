...
//	public ModelAndView boardpage1() {
//		return board(1);
//	}
	...
	public ModelAndView board(Integer page, String type, String search) {
		...
		HashMap<String, Object> ret = bs.getBoardList(page,type,search);
		mav.addObject("list", ret.get("list"));
		mav.addObject("pageCount", ret.get("pageCount"));
		mav.addObject("page", ret.get("page"));
		mav.addObject("begin", ret.get("begin"));
		mav.addObject("end", ret.get("end"));
		return mav;
	}
	@GetMapping("/board/{idx}")	// board 뒤에 어떤 값이 추가로 들어오면 글번호로 특정 글을 불러온다
	public ModelAndView view(@PathVariable int idx, Integer page, String type, String search, @RequestHeader("referer") String referer, HttpServletRequest request) {	// 경로값을 변수로 사용한다
